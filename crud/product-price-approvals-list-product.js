...
function GetArticoliApprovazione(myRequest) {
        ...
        try {
            ...
            soap.createClient(config.SapWebServiceURL, function (err, client) {
                if (err) {
                    ...
                    client.GetArticoliApprovazioni({ myLanguageContext, myOffsetRows, myNextRows, myCodiceFornitore, myCodiceArticolo, myIdApprovazione }, function (err, result) {
                        if (err) {
                            reject(JSON.stringify(
                                new exception(sender, err.message, err.name, err.stack))
                            );
                            ...
                            var resultStatus = response.GetArticoliApprovazioniResult.Status;
                            var resultError = response.GetArticoliApprovazioniResult.Error;

                            /* Ottiene il numero totale dei record restiuiti dal Web Service */
                            var resultRowsCount = eval(response.GetArticoliApprovazioniResult.RowsCount);

                            if (resultStatus == "OK") {

                                /* Inizializza i dati ricevuti dal Web Service */
                                var resultData = response.GetArticoliApprovazioniResult.Result.diffgram.Data.Articolo;
...
function PutSwitchProductApprovalsState(myRequest) {

    const sender = arguments.callee.name;

    var myIdAccount = myRequest.IdAccount;
    var myIdApprovazioneArticolo = myRequest.IdApprovazioneArticolo;
        ...
        try {
            sql.connect(connection, function (err) {
                if (err) {
                    ...
                    request.input('IdApprovazioneArticolo', sql.Int, myIdApprovazioneArticolo);
                    request.input('IdStatoApprovazione', sql.Int, myIdStatoApprovazione);
                    request.input('Owner', sql.NVarChar(50), myIdAccount);
                    request.output('Status', sql.NVarChar(2))

                    request.execute("SP_PutSwitchProductApprovalsState", function (err, response) {
                        if (err) {
                            reject(
                                JSON.stringify(new exception(sender, err.message, err.name, err.stack))
                            );
...
module.exports = {
    GetArticoliApprovazione,
    GetArticoliById,
    GetBuyer,
    GetFornitori,
    PostDetectPrice,
    PostSuggestPrice,
    PutSwitchProductApprovalsState,
