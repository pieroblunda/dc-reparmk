...
const attempLogin = async (creds) => {
  const sender = 'login.js - attempLogin';

  const email = creds.Userid;
  ...
  try {
    ...
    const result = await pool.request()
      ...
      .query(`
        SELECT
            (SELECT STRING_AGG(CONVERT(varchar(MAX), email) , ',') FROM users WHERE role = 'SUPERVISOR') AS Supervisor,
            u.id AS Id,
            u.email AS Username,
      ...
      let languageContext = config.LanguageContext;
    ...
    throw new Error(JSON.stringify(excep));
  }
}

module.exports = { login, attempLogin };
